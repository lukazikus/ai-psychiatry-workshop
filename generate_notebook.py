...
'\U0001f4dd <b>Edit the <code>DATA_ROOT</code></b> below to point to your E-DAIC dataset folder on Google Drive.\n'
'</div>'
))
...
'\n'
"# Participant folder pattern: {pid}_P  (e.g., 300_P, 301_P, ...)\n"
...
'    print(f"   Sample: {contents[:8]}")\n'
'else:\n'
'    print(f"\u274c DATA_ROOT not found: {DATA_ROOT}")\n'
'    print("   Please update DATA_ROOT to point to your E-DAIC folder.")'
))
...
cells.append(code(
'# ---- Load train / dev / test label CSVs ----\n'
...
'sample_folder = os.path.join(DATA_ROOT, PARTICIPANT_FOLDER.format(pid=sample_pid))\n'
'\n'
...
'    sample_folder,\n'
'    TRANSCRIPT_FILE.format(pid=sample_pid)\n'
')\n'
'\n'
...
'    folder = os.path.join(data_root, PARTICIPANT_FOLDER.format(pid=pid))\n'
'    tpath = os.path.join(folder, TRANSCRIPT_FILE.format(pid=pid))\n'
'\n'
'    if not os.path.exists(tpath):\n'
'        # Try auto-detect\n'
"        possible = glob.glob(os.path.join(folder, '*[Tt]ranscript*'))\n"
'        if possible:\n'
'            tpath = possible[0]\n'
'        else:\n'
'            return None\n'
...
'    folder = os.path.join(data_root, PARTICIPANT_FOLDER.format(pid=pid))\n'
'    apath = os.path.join(folder, AUDIO_FEATURE_FILE.format(pid=pid))\n'
'\n'
'    if not os.path.exists(apath):\n'
"        possible = glob.glob(os.path.join(folder, '*openSMILE*')) + \\\n"
"                   glob.glob(os.path.join(folder, '*OpenSMILE*')) + \\\n"
"                   glob.glob(os.path.join(folder, '*egemaps*')) + \\\n"
"                   glob.glob(os.path.join(folder, '*eGeMAPS*'))\n"
'        if possible:\n'
'            apath = possible[0]\n'
'        else:\n'
...
'    folder = os.path.join(data_root, PARTICIPANT_FOLDER.format(pid=pid))\n'
'    vpath = os.path.join(folder, VISUAL_FEATURE_FILE.format(pid=pid))\n'
'\n'
'    if not os.path.exists(vpath):\n'
"        possible = glob.glob(os.path.join(folder, '*openFace*')) + \\\n"
"                   glob.glob(os.path.join(folder, '*OpenFace*')) + \\\n"
"                   glob.glob(os.path.join(folder, '*Pose*gaze*AU*')) + \\\n"
"                   glob.glob(os.path.join(folder, '*AU*'))\n"
'        if possible:\n'
'            vpath = possible[0]\n'
'        else:\n'
'            return None\n'
